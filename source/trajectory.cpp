...
void trajectory::Squat(double t, TrunkModel_* trunk_model)
{
    ...
    {
        trunk_model->pos_trunk_des[0] = squat_r0;
        trunk_model->pos_trunk_des[1] = 0; // set alpha as 0 rad
        trunk_model->pos_trunk_des[2] = 0; // set beta as 0 rad
    }
    else if (t1 <= t_norm && t_norm < t2)
    {
        trunk_model->pos_trunk_des[0] = squat_r0 - 0.5 * deltaR * (1 - cos(2 * pi * freq_squat * (t_norm - t1)));
        trunk_model->pos_trunk_des[1] = 0; // set alpha as 0 rad
        trunk_model->pos_trunk_des[2] = 0; // set beta as 0 rad 
        trunk_model->vel_trunk_des[0] = - deltaR * (pi * freq_squat) * sin(2 * pi * freq_squat * (t_norm - t1));
    }
};

void trajectory::Roll(double t, TrunkModel_* trunk_model)
{
    double deltaR = roll_r0 - roll_rc;

    double T_roll = 1 / freq_roll;
    double T_period = T_roll + roll_T_pause + T_roll + roll_T_pause;

    double t_norm = t - T_period * floor(t / T_period); // nominalized time
    double t1 = roll_T_pause;
    double t2 = t1 + T_roll;
    double t3 = t2 + roll_T_pause;
    double t4 = t3 + T_roll;

    if (0 <= t_norm && t_norm < t1)
    {
        trunk_model->pos_trunk_des[0] = 0.3;
        trunk_model->pos_trunk_des[1] = 0; // set alpha as 0 rad
        trunk_model->pos_trunk_des[2] = 0; // set beta as 0 rad
    }
    else if (t1 <= t_norm && t_norm < t2)
    {
        trunk_model->pos_trunk_des[0] = 0.3;
        trunk_model->pos_trunk_des[1] = roll_r0 - 0.5 * deltaR * (1 - cos(2 * pi * freq_roll * (t_norm - t1)));
        trunk_model->pos_trunk_des[2] = 0; // set beta as 0 rad 
        trunk_model->vel_trunk_des[1] = - deltaR * (pi * freq_roll) * sin(2 * pi * freq_roll * (t_norm - t1));
    }
    else if (t2 <= t_norm && t_norm < t3)
    {
        trunk_model->pos_trunk_des[0] = 0.3;
        trunk_model->pos_trunk_des[1] = 0; // set alpha as 0 rad
        trunk_model->pos_trunk_des[2] = 0; // set beta as 0 rad
    }
    else if (t3 <= t_norm && t_norm < t4)
    {
        trunk_model->pos_trunk_des[0] = 0.3;
        trunk_model->pos_trunk_des[1] = roll_r0 + 0.5 * deltaR * (1 - cos(2 * pi * freq_roll * (t_norm - t3)));
        trunk_model->pos_trunk_des[2] = 0; // set beta as 0 rad 
        trunk_model->vel_trunk_des[1] = deltaR * (pi * freq_roll) * sin(2 * pi * freq_roll * (t_norm - t3));
    }
};

void trajectory::Pitch(double t, TrunkModel_* trunk_model)
{
    double deltaR = pitch_r0 - pitch_rc;

    double T_pitch = 1 / freq_pitch;
    double T_period = T_pitch + pitch_T_pause + T_pitch + pitch_T_pause;

    double t_norm = t - T_period * floor(t / T_period); // nominalized time
    double t1 = pitch_T_pause;
    double t2 = t1 + T_pitch;
    double t3 = t2 + pitch_T_pause;
    double t4 = t3 + T_pitch;
    ...
    {
        trunk_model->pos_trunk_des[0] = 0.3;
        trunk_model->pos_trunk_des[1] = 0;
        trunk_model->pos_trunk_des[2] = pitch_r0 - 0.5 * deltaR * (1 - cos(2 * pi * freq_pitch * (t_norm - t1))); // set beta as 0 rad 
        trunk_model->vel_trunk_des[2] = - deltaR * (pi * freq_pitch) * sin(2 * pi * freq_pitch * (t_norm - t1));
    }
    else if (t2 <= t_norm && t_norm < t3)
    {
        trunk_model->pos_trunk_des[0] = 0.3;
    ...
    else if (t3 <= t_norm && t_norm < t4)
    {
        trunk_model->pos_trunk_des[0] = 0.3;
        trunk_model->pos_trunk_des[1] = 0;
        trunk_model->pos_trunk_des[2] = pitch_r0 + 0.5 * deltaR * (1 - cos(2 * pi * freq_pitch * (t_norm - t3))); // set beta as 0 rad 
        trunk_model->vel_trunk_des[2] = deltaR * (pi * freq_pitch) * sin(2 * pi * freq_pitch * (t_norm - t3));
    }
};

void trajectory::Hold(TrunkModel_* trunk_model)
{
    trunk_model->pos_trunk_des[0] = 0.3536; // set y as 0.3536 m
    trunk_model->pos_trunk_des[1] = 0; // set alpha as 0 rad
    trunk_model->pos_trunk_des[2] = 0; // set beta as 0 rad
};
